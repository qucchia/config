...
#+begin_src emacs-lisp
  ...
  (defun lieutenant-app (command name class)
    (let ((choice
      (ivy-read (format "Switch to %s buffer:" name) (append
        (cl-mapcar (lambda (buffer) (buffer-name buffer))
          (seq-filter (lambda (buffer) (when
            (string= (buffer-local-value 'exwm-class-name buffer) class)
              buffer))
            (buffer-list)))
        '("New process")))))
        (if (string= choice "New process")
          (start-process-shell-command name (format "*%s log*" name) command)
          (switch-to-buffer choice))))

  (defmacro lieutenant-app-command (command name class which-key)
    (list 'list
      (list 'lambda (list)
        (list 'interactive)
